...
public class NeoSampleService extends Service {
    private static final String TAG = "neosampleservice";
...
//    private DbOpenHelper mDbOpenHelper;
    ...
    {
        super.onCreate();
        Log.d(TAG, "Started Processing the Service");
        ...
        float r = 3.44f;
        float b = 3.44f;
        mrectF = new RectF(4.55f,4.55f,4.55f,4.55f);
        ...
        formIntent = new Intent(this, PopulateField.class);
//        manifestIntent = new Intent(this, ManifestActivity.class);
...
//        mDbOpenHelper.create();
        ...
        Log.d(TAG,  "Service Initialize complete" );
    }
...
//            enqueueDotForBroadcast(macAddress, dot);
    ...
    private IOfflineDataListener mOfflineDataListener = new IOfflineDataListener()
    {
        ...
        {
            Log.d(TAG, "Receiced Some Offline Stroke");
            ...
            for(Stroke s : strokes)
            {
                s.getDots();
                if(s.size() > 0 ) {
                    Log.d(TAG, "Offline content: " + s.toString());
                    NLog.d( "onReceiveOfflineStrokes s sectionId="+s.sectionId+" ownerId="+s.ownerId+" noteId="+s.noteId+" pageId="+s.pageId );
                    Log.d(TAG, "onReceiveOfflineStrokes s sectionId=" + s.sectionId + " ownerId=" + s.ownerId + " noteId=" + s.noteId + " pageId=" + s.pageId +
                            (s.contains(new RectF(10.714f,10.714f,21.429f,21.429f)) ? "Contains" : "Does Not") + " Type=" + s.type);
                    s.color = Color.BLACK;
                    newArrayList.add(s);

                    // DB에 stroke 를 저장한다.
                    // DB Insert
                    mDataSource.insertStroke(s);
                    Log.d(TAG, "NewHere");
...
//            if(Const.DOC_TYPE== "form"){
//                formIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
//                startActivity(formIntent);
//            }
//            if(Const.DOC_TYPE=="manifest"){
//                manifestIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
